...
# Ciclo de consolidação de estoques mensais
...
# Impondo Trimestres
ESTOQUE <- 
  pivot_longer(ESTOQUE, cols = c(3:27), names_to = "MES")

ESTOQUE$TRIMESTRE <- 
  lubridate::ym(ESTOQUE$MES) |> lubridate::quarter(type = "year.quarter")

# Sumarizando por trimestre

ESTOQUE <- 
  summarise(
    group_by(
      ESTOQUE, cnae20subclas, codmun, TRIMESTRE), 
    "value" = sum(value)) 

# Long to Wide
ESTOQUE <- 
  pivot_wider(ESTOQUE, names_from = TRIMESTRE, values_from = value)


# Unindo Suclasses Descrição / Municípios
...
ESTOQUE <- 
  ESTOQUE[,c("cnae20subclas", "2020.1", "2020.2", "2020.3", "2020.4", 
             "2021.1", "2021.2", "2021.3", "2021.4", "2022.1", "subclasse.descrição", 
             "classe.descrição", "grupo.descrição", 
             "divisão.descrição", "seção.descrição", 
             "Município", "UF_sigla")]

colnames(ESTOQUE) <- c("subclasse_Cod", "2020.1", "2020.2", "2020.3", "2020.4", 
                       "2021.1", "2021.2", "2021.3", "2021.4", "2022.1", 
                       "subclasse", "classe", "grupo", "divisão", "seção", 
                       "Município", "UF_sigla"
                       )

saveRDS(ESTOQUE, "./data/Estoque_Novo_Caged_Consolidacao_Saldo_TRIMESTRE_20221030.RDS")
