...
movimentacao_CNAE_B_C$competência <-
  movimentacao_CNAE_B_C$competência |> lubridate::as_date() |> zoo::as.yearmon()
...
df_movimentacao <-
  movimentacao_CNAE_B_C[, c(
    ...
    "Município",
    "NO_CBO",
    "tipomovimentação.Descrição",
    "graudeinstrução.Descrição",
    "tamestabjan.Descrição",
    "tipoempregador.Descrição",
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    width = 100,
    sidebarMenu(
      menuItem("Estoques", tabName = "Estoques"),
      ...
      tabItem(
        ...
        sidebarLayout(
          sidebarPanel = sidebarPanel(fluid = T,
                                  ...
                                  #                     value = movimentacao_CNAE_nB_C$competência  ,
                                    ...
                                    tabBox(
                                      ...
                                      tabPanel(title = "MUNICÍPIOS",
                                               box(
                                                 ...
                                                 collapsed = T,
                                                 checkboxGroupInput(
                                                   inputId = "id.MunicipioCheckboxGroupInput",
                                                   label = "",
                                                   choices = MUNICIPIOS,
                                                   selected = c("")
                                                 )
                                               )),
                                      tabPanel(title = "UF_Estados",
                                               box(
                                                 title = "UF",
                                                 width = NULL,
                                                 collapsible = T,
                                                 ...
                                                 checkboxGroupInput(
                                                   width = 2,
                                                   inputId = "id.UfCheckboxGroupInput",
                                                   label = "",
                                                   choices = UF,
                                                   selected = c("MG")
                                                 )
                                ...
                                mainPanel = mainPanel(
                                  ...
                                  fluidRow(
                                    box(
                                      title = "Admissões",
                                      textOutput(outputId = "id.Admissoes"),
                                      width = 2
                                    ),
                                    box(
                                      title = "Desligamentos",
                                      textOutput(outputId = "id.Desligamentos"),
                                      width = 2
                                    ),
                                    box(
                                      title = "Saldo",
                                      textOutput(outputId = "id.Saldo"),
                                      width = 2
                                    )
                                  ),
                                  # ________ FluidRow 2 ----
  ...
  server <- function(input, output, session) {
    ...
    MUNICIPIOS <-
      observe(df_long_cnae[df_long_cnae$UF_sigla %in% "id.UfCheckboxGroupInput",])
    ...
    output$id.TabelaCNAE <- renderReactable({
      ...
      df_long <-
        df_long_cnae[df_long_cnae$trimestre %in% input$id.TrimestreSliderTextInput, ]
      
      reactable(
        compact = T,
        columns = list(
          secao = colDef(name = "Nível cnae 2.0"),
          estoque = colDef(name = "Estoque")
        ),
        highlight = T,
        data = summarise(group_by(df_long, secao), "estoque" = sum(estoque)),
        details = function(index) {
          df_divisao <-
            df_long[df_long$secao == sort(unique(df_long$secao))[index], c("divisao", "grupo", "classe", "subclasse", "estoque")]
          htmltools::div(
            reactable(
              compact = T,
              columns = list(
              ...
              highlight = T,
              data = summarise(group_by(df_divisao, divisao), "estoque" = sum(estoque)),
              details = function(index) {
                df_grupo <-
                  df_divisao[df_divisao$divisao == sort(unique(df_divisao$divisao))[index], c("grupo", "classe", "subclasse", "estoque")]
                htmltools::div(
                  reactable(
                    compact = T,
                    columns = list(
                    ...
                    highlight = T,
                    data = summarise(group_by(df_grupo, grupo), "estoque" = sum(estoque)),
                    details = function(index) {
                      df_classe <-
                        df_grupo[df_grupo$grupo == sort(unique(df_grupo$grupo))[index], c("grupo", "classe", "subclasse", "estoque")]
                      htmltools::div(
                        reactable(
                          compact = T,
                          columns = list(
                          ...
                          highlight = T,
                          data = summarise(group_by(df_classe, classe), "estoque" = sum(estoque)),
                          details = function(index) {
                            df_subclasse <-
                              df_classe[df_classe$classe == sort(unique(df_classe$classe))[index], c("classe", "subclasse", "estoque")]
                            htmltools::div(reactable(
                              compact = T,
                              columns = list(
                                subclasse = colDef(name = ""),
                                estoque = colDef(name = "")
                              ),
                              highlight = T,
                              data = summarise(
                                group_by(df_subclasse, subclasse),
                                "estoque" = sum(estoque))))}))}))}))})})
    ...
    output$id.PlotCNAEGrupos <- renderPlot({
      ...
      df_long <-
        df_movimentacao[df_movimentacao$trimestre %in% input$id.TrimestreSliderTextInput, ]
      
      df_divisao <-
        summarise(group_by(df_long, divisao), "saldo" = sum(saldomovimentacao))
      
      ggplot(data = df_divisao) +
        geom_col(mapping = aes(x = as.factor(divisao), y = saldo),
                 fill = "#FFCC00") +
        xlab("") + ylab("") +
        scale_x_discrete(labels = str_wrap(string = as.factor(unique(
          df_divisao$divisao
        )), width = 40)) +
